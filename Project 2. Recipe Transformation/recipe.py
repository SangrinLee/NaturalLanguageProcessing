...
"rabbit", "rib", "ribs"
"salmon", "sardine", "sardines", "sausage", "shrimp", "shrimps", "steak",
"trout", "tuna", "turkey", 
"veal", "venison"]
...
def transform(orig, sel):
	...
	if a != 0 or b != 0:
		...
		orig["ingredients"][b]["item"] = [sauce_sel]
		orig["ingredients"][b]["type"] = ["vegetarian"]
		# print "##### STEP LIST #####"
		# print original_item
		# print " ".join(original_item)
		temporary = " ".join(original_item)
		# print sauce_sel
		# for i in orig["step"]:
		# 	print i
		# 	if temporary in i or temporary.lower() in i:
		# 		i.replace()
		# 		print "FOUND!!!!!"
		update_steps_in_tool_transform([temporary],sauce_sel, orig["step"])	
# lower_ingredient.replace(protein,protein_rpc_list[random_index])
...
#   glb_methods_list              =list(orig["methods"]) 
	...
	for index, value in glb_ingredients_dic.iteritems(): 
	    ...
	    merged_item=" ".join(value["type"])
	    glb_type_list.append(merged_item)
	    ...
	    for index,vaule in enumerate(glb_ingredients_list):
			try:
				...
				if protein in lower_ingredient:
					...
					glb_ingredients_list[index]=protein_rpc_list[random_index]
					glb_type_list[index]="vegetarian"
					update_steps_in_tool_transform([protein],protein_rpc_list[random_index], orig["step"])	
	...
	if c == True:
		...
		for i in orig["ingredients"]:
			...
			orig["ingredients"][i]["item"] = glb_ingredients_list[i-1].split()
			orig["ingredients"][i]["type"] = glb_type_list[i-1].split()
			# print orig["ingredients"][i]["item"]
			# print glb_ingredients_list[i-1]
			# print orig["ingredients"][i]["type"]
			# print glb_type_list[i-1]
	...
	# jdata = orig["ingredients"]

	# json_data = {}
	# json_data.update({'quantity': "1"})
	# json_data.update({'measures': ["teaspoon"]})
	# json_data.update({'desc': "none"})
	# chosen = []
	# chosen.append(sel)
	# json_data.update({'item': chosen})
	# json_data.update({'prep': "none"})
	# if sel in vegan_sauce:
	# 	json_data.update({'type': ["vegan"]})
	# elif sel in vegi_sauce:
	# 	json_data.update({'type': ["vegetarian"]})
	# else:
	# 	json_data.update({'type': ["nonveg"]})
	# print str(len(jdata))
	# temp = {len(jdata) + 1 : json_data}
	# orig["ingredients"].update(temp)

	return orig


def transform1(orig, sel):
	print "########## Recipe Transformation ##########"
	if sel == "1":
		print "you chose 1"
	a = 0
...
def transformfromVegToNonVeg(orig, sel):
	...
	for i in orig:
		for j in orig[i]:
			...
			if i == "types":
				# print "##### Types of Ingredient #####"
				if orig[i][j] == "vegetarian" or orig[i][j] == "vegan":
					print "Non-Vegetarian Source List"
			...
			if i == "ingredients":
				...
				for k in orig[i][j]:
					if k == "type":
						if len(orig[i][j][k]) > 0:
							if orig[i][j][k][0] == "vegetarian" or orig[i][j][k][0]:
								original_item = orig[i][j]["item"]
								a = i
								b = j
	...
	if a != 0 or b != 0:
		...
		# temp2.append(sauce_sel)
		orig["ingredients"][b]["item"] = [sauce_sel]
		orig["ingredients"][b]["type"] = ["nonveg"]
		# print "##### STEP LIST #####"
		# print original_item
		# print " ".join(original_item)
		temporary = " ".join(original_item)
		...
		for index,vaule in enumerate(glb_ingredients_list):
			try:
				...
				if protein in lower_ingredient:
					...
					update_steps_in_tool_transform([protein],protein_list[random_index], orig["step"])	
...
def showRecipe(orig):
	...
	for i in orig:
		for j in orig[i]:
			...
			if i == "ingredients":
				print ""
				print "- Ingredient " + str(j)
				for k in orig[i][j]:
					if k == "item":
						print "Name : " + ' '.join(str(p) for p in orig[i][j][k]) 
					if k == "type":
						print "Type : " + ' '.join(str(p) for p in orig[i][j][k]) 
					
 					# meaureQua=''
					if k == "quantity":
						# measureQua = "Quantity : " + ' '.join(str(p) for p in orig[i][j][k]) 
						print "Quantity : " + ' '.join(str(p) for p in orig[i][j][k])
					if k == "measures":
						# measureQua += orig[i][j][k]
						print "Measurement : " + ' '.join(str(p) for p in orig[i][j][k])
					if k == "prep":
						print "Preparation : " + ' '.join(str(p) for p in orig[i][j][k])
					if k == "desc":
						print "Description : " + ' '.join(str(p) for p in orig[i][j][k])
...
def getHealthy(original_recipe, url):
...
def bake2fry(orig):
	ingredient_list = []
	for i in orig["ingredients"]:
		ingredient_list.append(orig["ingredients"][i]["item"])

	toollist = orig["tools"][1]
	methodlist = orig["methods"][1]
	directionlist = orig["step"]
	...
	ingre_quantitylist = []
	for i in orig["ingredients"]:
		ingre_quantitylist.append(orig["ingredients"][i]["quantity"])
	ingre_quantitylist=Updata_Quantity_list(protein_list,ingredient_list,ingre_quantitylist, True)

	orig["tools"][1] = toollist
	orig["methods"][1] = methodlist
	orig["step"] = directionlist
	for i in orig["ingredients"]:
		orig["ingredients"][i]["quantity"] = ingre_quantitylist[i-1]
...
# input_url = "http://allrecipes.com/recipe/158587/smoky-four-cheese-macaroni-bake" ## Demo
# input_url = "http://allrecipes.com/recipe/37965/asian-fire-meat" ## TEST
# input_url = "http://allrecipes.com/recipe/202975/pot-stickers-chinese-dumplings"
input_url = "http://allrecipes.com/recipe/13185/chinese-spicy-hot-and-sour-soup"
...
while 1:
	print "\n####################### MAIN MENU ###########################"
	print "\n########## Select DIY to easy ( 1, 2, 3, 4, 5, 6 ) ##########"
	print "\n#############################################################"
	...
	if sel == "1":
		while 1:
			print "\n### Select Vegetarian / Non-vegetarian ( 1, 2, 3 ) ###"
			print "1 - From Non-Vegetarian/Non-Vegan To Vegetarian/Vegan"
			print "2 - From Vegetarian/Vegan To Non-Vegetarian/Non-Vegan"
			print "3 - Back to Main Menu"
			sel1 = raw_input("Select :  ")
			if sel1 == "1":
				transformed_recipe = transform(original_recipe, sel)
				print "==================== TRANSFORMED RECIPE ===================="
				showRecipe(transformed_recipe)
			elif sel1 == "2":
				transformed_recipe = transformfromVegToNonVeg(original_recipe, sel)
				showRecipe(transformed_recipe)
			elif sel1 == "3":
				print ""
				break
		...
		while 1:
			...
			if int(sel2) in range(0, len(all_cuisine_list)):
				...
				while 1:
					...
					sel22 = raw_input("Please Enter the Cuisine Name (Case Sensitive) : ")
					if sel22 == "-1":
						break
					elif sel22 in all_cuisine_list[int(sel2)]:
			...
			print "\n### Healthy Recipe ###"
			transformed_recipe3 = getHealthy(original_recipe, input_url)
			print "==================== TRANSFORMED RECIPE ===================="
			showRecipe(transformed_recipe3)
			# break
	elif sel == "4":
		while 1:
			print "\n### Select DIY to easy ( 1, 2, 3 ) ###"
			print "1 - easy"
			print "2 - super easy"
			print "3 - Back to Main Menu"

			sel4 = raw_input("Select : ")
			if sel4 == "1":
		...
		while 1:
			print "\n### Select Cooking Method to Change ( 1, 2, 3, 4 ) ###"
			print "1 - Bake to Fry"
			print "2 - Fry to Bake"
			print "3 - Roast to Fry"
			print "4 - Fry to Roast"
			print "5 - Back to Main Menu"
			sel5 = raw_input("Select : ")
			if sel5 == "1":
				transformed_recipe5 = bake2fry(original_recipe)
				showRecipe(transformed_recipe5)
			elif sel5 == "2":
				transformed_recipe5 = fry2bake(original_recipe)
				showRecipe(transformed_recipe5)
			elif sel5 == "3":
				transformed_recipe5 = roast2fry(original_recipe)
				showRecipe(transformed_recipe5)
			elif sel5 == "4":
				transformed_recipe5 = fry2roast(original_recipe)
				showRecipe(transformed_recipe5)
			elif sel5 == "5":
				print ""
				break	
